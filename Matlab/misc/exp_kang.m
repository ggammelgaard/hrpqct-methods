...
% Start logfile
log_name = append(path_logs, datestr(now,'yymmdd_HHMMSS'), "_exp_kang.txt");
diary(log_name);
...
for i = 1:n_combinations
    ...
    for j = 1:n_files
        ...
        % Compute thresholder erosion map
        data = preprocessing(data);
        data = segmentation(data);
        data = registration(data);
        data = volume_cleaning(data);
        data = detection(data);
        data.labelmap = int8(logical(data.erosions))*3;
        data = deregister(data);
        ero_pred = logical(data.labelmap);
        % Load GT ero map
        ero_true = load_ero_map(data);
        % Calc tp, fp, and fn score
        tmp_tp = sum(ero_true(:) .* ero_pred(:));
        tmp_fn = sum(ero_true(:)) - tmp_tp;
        tmp_tn = sum(~ero_true(:) .* ~ero_pred(:));
        tmp_fp = sum(~ero_true(:)) - tmp_tn;
        % Update ouside vars
