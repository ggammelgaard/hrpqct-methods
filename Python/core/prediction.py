...
from .generator import get_non_healthy_class_labels
...
def get_prediction_labels(prediction, activation='softmax', threshold=0.5, labels=None):
    n_samples = prediction.shape[0]
    label_arrays = []
    # print("prediction.shape:", prediction.shape, "| n_samples:", n_samples)
    # pretty_prediction1 = ["{:.4f}".format(i) for i in prediction[0][:, 5, 5, 5]]
    # pretty_prediction2 = ["{:.4f}".format(i) for i in prediction[0][:, 65, 65, 65]]
    # print("prediction[0][:, 5, 5, 5]", pretty_prediction1)
    # print("prediction[0][:, 65, 65, 65]", pretty_prediction2)
    for sample_number in range(n_samples):
        # if activation == 'softmax':
        label_data = np.argmax(prediction[sample_number], axis=0)  # predictions are flipped
...
def prediction_to_image(prediction, affine, label_map=False, activation='softmax', threshold=0.5, labels=None,
                        non_healthy=False):
    if prediction.shape[1] == 1:
        ...
        if label_map:
            label_map_data = get_prediction_labels(prediction, activation=activation, labels=labels)
    ...
    if non_healthy:
        data = data * 4  # make output blue in Slicer
...
def run_validation_case(data_index, output_dir, model, data_file, training_modalities, activation='softmax',
    ...
    test_name = data_file.root.subject_ids[data_index].decode('ascii')
    for i, modality in enumerate(training_modalities):
        image = nib.Nifti1Image(test_data[0, i], affine)
        image.to_filename(os.path.join(output_dir, "{}_data.nii.gz".format(test_name)))

    # Save the ground truth map
    test_truth = nib.Nifti1Image(data_file.root.truth[data_index][0], affine)
    test_truth.to_filename(os.path.join(output_dir, "{}_truth.nii.gz".format(test_name)))

    # Save the healthy / non-healthy truth map
    if non_healthy:
        truth_map = data_file.root.truth[data_index][0]
        truth_map = get_non_healthy_class_labels(truth_map) * 4  # multiply with 4 to get blue color in slicer
        test_truth = nib.Nifti1Image(truth_map, affine)
        test_truth.to_filename(os.path.join(output_dir, "{}_hnh.nii.gz".format(test_name)))  # healthy / non-healthy img
    ...
    prediction_image = prediction_to_image(prediction, affine, activation=activation, label_map=output_label_map,
                                           threshold=threshold, labels=labels, non_healthy=non_healthy)
    if isinstance(prediction_image, list):
        for i, image in enumerate(prediction_image):
            image.to_filename(os.path.join(output_dir, "{}_prediction_{}.nii.gz".format(test_name, i + 1)))
    else:
        prediction_image.to_filename(os.path.join(output_dir, "{}_prediction.nii.gz".format(test_name)))


def run_validation_cases(validation_keys_file, model_file, training_modalities, labels, hdf5_file, activation='softmax',
                         output_label_map=False, output_dir=".", threshold=0.5, overlap=16, permute=False,
                         non_healthy=False):
    # Accepts both a list, and a path to a pickled list
    if type(validation_keys_file) == list:
        validation_indices = validation_keys_file
    else:
    ...
    print("Number of validation files: {}".format(len(validation_indices)))
    i_status = 1

    for index in validation_indices:
        print("Predicting file no. {}: Index {} - {}".format(i_status, index,
                                                             data_file.root.subject_ids[index].decode('utf-8')))
        ...
        run_validation_case(data_index=index, output_dir=case_directory, model=model, data_file=data_file,
                            ...
                            threshold=threshold, overlap=overlap, permute=permute, activation=activation,
