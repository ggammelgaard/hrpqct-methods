...
from core.utils.utils import resize, read_image_files
...
def get_cropping_parameters(in_files):
    # print("Hello from get_cropping_parameters")
    if len(in_files) > 1:
...
def get_foreground_from_set_of_files(set_of_files, background_value=0, tolerance=0.00001, return_image=False):
    for i, image_file in enumerate(set_of_files):
        # print("Finding foreground of: {}".format(image_file))
    ...
    if return_image:
        hola = new_img_like(image, foreground)
        return hola
    else:
        return foreground
