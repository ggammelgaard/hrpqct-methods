...
now=$(date +'%y%m%d_%H%M')
experiment_folder="${now}_u$experiment_to_run"

# Make directory to add nohup output log
SCRIPT=`realpath $0`
SCRIPTPATH=`dirname $SCRIPT`
MAINPATH=`dirname $SCRIPTPATH`
output_path="$MAINPATH/output_unet/$experiment_folder"
...
# Start script
command3="nohup python -u $MAINPATH/unet_experiments/$experiment_to_run.py $kfold_subfolder $experiment_folder > $output_path/console.txt &"
